...
let cities = city.map((element) => {
  let e = `${element.city}, ${element.state}`;
  return e;
});
...
searchedCity.addEventListener("input", () => {
  for (let i = 0; i < data.length; i++) {
    if (
      data[i].city.toLocaleLowerCase() ===
      searchedCity.value.toLocaleLowerCase()
    ) {
      latitude = data[i].lat;
      longitude = data[i].lng;
      document.querySelector(
        ".location"
      ).innerHTML = `${data[i].city},</br>${data[i].state}`;
...
async function GetWeather(latitude, longitude) {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            resolve();
            // This callback handles when user denies access and instead of calling reject to throw error we resolve
          },
          () => {
            latitude = 41.3275; // Set default latitude Tirana
            longitude = 19.8188896; // Set default longitude Tirana
            document.getElementById("error-message").textContent =
              "This device denied location services or it is turned off. Default Location Tirana";
            resolve(); // Resolve the promise with default values Tirana
          }
        );
      }
    });
  ...
  try {
    ...
    errorMessage.textContent =
      "You need to allow location services to get weather!Try reload and click yes for location";
  }
  ...
  document.getElementById(
    "sunset0"
  ).innerHTML = `<h3>Sunrise at ${morning}</h3>Sunset at ${evening}`;
  ...
  document.getElementById("day0").innerHTML =
    week[(currenTime.getDay() + 1) % 7];
  document.getElementById("day1").innerHTML =
    week[(currenTime.getDay() + 2) % 7];
  document.getElementById("day2").innerHTML =
    week[(currenTime.getDay() + 3) % 7];
  document.getElementById("day3").innerHTML =
    week[(currenTime.getDay() + 4) % 7];
  // display Month
  document.getElementById("date-num").innerHTML = `${
    month[currenTime.getMonth()]
  }, ${currenTime.getDate()}`;
  ...
  for (let i = 0; i < todayHours.length; i++) {
    if (currenTime.getHours() == todayHours[i]) {
      document.getElementById(
        "today-temp"
      ).innerHTML = `${days[0].temp[i]} ${hourly_units.temperature_2m}`;
      document.getElementById(
        "prc-value"
      ).innerHTML = `${days[0].prc[i]} ${hourly_units.precipitation}`;
      document.getElementById(
        "wnd-value"
      ).innerHTML = `${days[0].wnd[i]} ${hourly_units.windspeed_10m}`;
      document.getElementById(
        "clouds"
      ).innerHTML = `${days[0].cloud[i]} ${hourly_units.cloudcover}`;
      document.getElementById(
        "day0-temp"
      ).innerHTML = `${days[1].temp[i]} ${hourly_units.temperature_2m}`;
      document.getElementById(
        "day1-temp"
      ).innerHTML = `${days[2].temp[i]} ${hourly_units.temperature_2m}`;
      document.getElementById(
        "day2-temp"
      ).innerHTML = `${days[3].temp[i]} ${hourly_units.temperature_2m}`;
      document.getElementById(
        "day3-temp"
      ).innerHTML = `${days[4].temp[i]} ${hourly_units.temperature_2m}`;
      document.getElementById(
        "day1Prc"
      ).innerHTML = `${days[1].prc[i]} ${hourly_units.precipitation}`;
      document.getElementById(
        "day2Prc"
      ).innerHTML = `${days[2].prc[i]} ${hourly_units.precipitation}`;
      document.getElementById(
        "day3Prc"
      ).innerHTML = `${days[3].prc[i]} ${hourly_units.precipitation}`;
      document.getElementById(
        "day4Prc"
